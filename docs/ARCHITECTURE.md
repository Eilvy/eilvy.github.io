...
2. 同为置顶按 pinOrder 升序
3. 其余按「有效更新时间」倒序
4. 更新时间相同时，按创建时间（pubDate）倒序
5. 仍相同则按 id 升序兜底
```

「有效更新时间」= `updatedDate ?? pubDate`。

**为什么需要 4、5 两个次级键**：只按更新时间排序时，同一天更新的文章会
「相等」。相等时 JS 的 `sort` 是稳定的，会保留集合返回顺序 —— 那依赖
`getCollection` 的遍历顺序，并非有意的规则，内容组织一变就可能整体调换。
实测本仓库就出现过这种情况：`[k8s] Service` 与 `OSI 网络链路` 两篇的
`updatedDate` 都是 `2026-09-18`，谁在前纯属巧合。
补上 4、5 后比较成为**全序**，任意两篇都有确定先后，不依赖输入顺序。
