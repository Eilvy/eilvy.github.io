...
import {
	...
	pluginTimelineScene,
	lifecycleScene,
} from '../../components/whiteboard/envoy-wasm.scenes';
...
<WhiteboardLoader
	elements={filterChainScene}
	label="HTTP 过滤器链完整时序：请求经 Listener 与 HTTP Connection Manager 进入过滤器链，按 http_filters 顺序过 route-guard、key-auth、scheduler、rate-limiter、ai-proxy、ai-log、router，再由 Cluster 转发到上游；响应沿同一链倒序返回"
	buttonText="点击加载过滤器链时序图"
	height={800}
/>
...
<WhiteboardLoader
	elements={pluginTimelineScene}
	label="单插件内部时间线：decode 阶段处理请求头与请求体后转发到上游；encode 阶段处理响应头，再按流式或非流式分支分别处理响应体，最后 onHttpStreamDone 收尾"
	buttonText="点击加载插件时间线图"
	height={600}
/>
...
<WhiteboardLoader
	elements={lifecycleScene}
	label="从写代码到在网关生效：开发期把 Go 源码编译为 .wasm 字节码，配置期在 envoy.yaml.tmpl 的 http_filters 中挂载并加载，运行期由 Envoy 按过滤器链顺序处理请求并转发到上游"
	buttonText="点击加载全流程总览图"
	height={680}
/>
