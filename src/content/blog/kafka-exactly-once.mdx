...
import {
	deliverySemanticsScene,
	producerDurabilityScene,
	idempotentProducerScene,
	transactionScene,
	endToEndScene,
} from '../../components/whiteboard/kafka-consistency.scenes';
...
<WhiteboardLoader
	elements={deliverySemanticsScene}
	label="三条投递语义对比：最多一次先提交 offset 再处理消息，崩溃会丢消息；至少一次先处理再提交，崩溃会重复处理；精确一次把处理与 offset 提交放进同一事务"
	buttonText="点击加载投递语义对比图"
	height={500}
/>
...
<WhiteboardLoader
	elements={producerDurabilityScene}
	label="生产端不丢：acks=all 等的是当前 ISR 而非全部副本。正常情况下 HW 取所有 ISR 的最小 LEO；但当 Follower 掉队使 ISR 缩水成 1 时，acks=all 只等 Leader 自己，此时必须靠 min.insync.replicas 兜底"
	buttonText="点击加载副本与 HW 图"
	height={680}
/>
...
<WhiteboardLoader
	elements={idempotentProducerScene}
	label="幂等生产者的去重机制：生产者用 PID=7、seq=0 发送，Broker 记录该序列号；响应丢失后生产者重试同一条消息，Broker 发现已见过 (7,0)，直接丢弃不重复写入"
	buttonText="点击加载幂等去重图"
	height={560}
/>
...
<WhiteboardLoader
	elements={transactionScene}
	label="Kafka 事务：协调者通过两阶段提交把业务数据写入、消费位移提交打包成原子单元；消费端用 isolation.level=read_committed 只读取已提交的消息，未提交或已回滚的会被过滤掉"
	buttonText="点击加载事务与隔离级别图"
	height={640}
/>
...
<WhiteboardLoader
	elements={endToEndScene}
	label="端到端不丢不重的配置清单：生产端要 acks=all、开启幂等、重试足够；Broker 要 replication.factor≥3、min.insync.replicas≥2、关闭 unclean 选举；消费端要关闭自动提交、处理完再提交、用 read_committed，并在业务侧做幂等"
	buttonText="点击加载端到端清单图"
	height={560}
/>
