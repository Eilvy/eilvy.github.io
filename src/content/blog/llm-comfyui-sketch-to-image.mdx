...
import {
	...
	ecosystemScene,
	roadmapScene,
} from '../../components/whiteboard/llm-comfyui.scenes';
...
<WhiteboardLoader
	elements={targetPipelineScene}
	label="目标链路：用户草图经 LLM 多模态识图、生成结构化控制参数，再调用 ComfyUI API 执行并返回出图"
	buttonText="点击加载目标链路图"
	height={660}
/>
...
<WhiteboardLoader
	elements={comfyWorkflowScene}
	label="ComfyUI 草图工作流：草图经 ControlNet 预处理与应用约束结构，再由 KSampler 采样、VAEDecode 出图；右侧是 ControlNet 模型与底模等旁路依赖"
	buttonText="点击加载工作流图"
	height={620}
/>
...
<WhiteboardLoader
	elements={handoffScene}
	label="从人工调试到 API 调用的衔接点：WebUI 搭好工作流后导出 API 格式 JSON，中间层加载模板并由 LLM 填参，最后 POST /prompt 提交"
	buttonText="点击加载衔接点图"
	height={720}
/>
...
<WhiteboardLoader
	elements={ecosystemScene}
	label="开源生态映射：草图输入、LLM 识图、LLM 工具调用、ControlNet 出图四个环节，各自对应可复用的现成开源项目"
	buttonText="点击加载开源生态图"
	height={440}
/>
...
<WhiteboardLoader
	...
	label="推荐落地路径：WebUI 搭工作流并调优，导出 API JSON 模板，写薄编排层，以 SD1.5 + ControlNet 起步，跑通后可选 SDXL 高质量模式"
	buttonText="点击加载落地路径图"
	height={440}
/>
